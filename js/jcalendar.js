...
jQuery.jcalendar = function() {
	...
	var _drawCalendar = function(dateIn, a, day, month, year) {
	  ...
	  var d;
		...
		jQuery('div.jcalendar-month').text(months[d.getMonth()]);
		...
		do {
  		...
  		for (var i=0; i<7; i++) {
  			...
  			if (curDay < 0 || curDay >= lastDay) {
  				...
  				dayStr = jQuery('<a class="jcalendar-day" href="" rel="'+ d +'">'+ (curDay+1) +'</a>').click(function(e) {
            if (_selectedDate) {
            ...
            day.val(new Date(_selectedDate.attr('rel')).getDate());
					...
					Heatmap.Load(_selectedDate.attr('rel'));
  					return false;
  				});
...
jQuery.fn.jcalendar = function(a) {
    ...
    jQuery('div.jcalendar-month').after('<div class="jcalendar"></div>');
